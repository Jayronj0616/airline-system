    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-6">Online Check-In</h1>
                ...
                <div class="bg-blue-50 dark:bg-blue-900/30 rounded-lg p-6 mb-8">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Flight Summary</h2>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold dark:text-white">{{ $booking->flight->departure_time->format('M d, Y H:i') }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Route</p>
                            <p class="text-lg font-semibold dark:text-white">{{ $booking->flight->origin }} → {{ $booking->flight->destination }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Passengers</p>
                            <p class="text-lg font-semibold dark:text-white">{{ $booking->passengers->count() }}</p>
                        </div>
                    </div>
                </div>
                        ...
                        @foreach($booking->passengers as $passenger)
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <p class="font-semibold dark:text-white">{{ $passenger->full_name }}</p>
                                        @if($passenger->seat)
                                            <p class="text-sm text-gray-600 dark:text-gray-400">Seat: {{ $passenger->seat->seat_number }}</p>
                                        @endif
                                    </div>
                ...
                <div class="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 rounded-lg p-6 mb-8">
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-2">Important Information</h3>
